describe StatusDashboard do
  subject { StatusDashboard }

  let (:status) { build_stubbed :status }

  it "defines attribute types" do
    expect(subject.const_get(:ATTRIBUTE_TYPES).length).to eq(9)
  end
  ...
  it "defines show page attributes" do
    expect(subject.const_get(:SHOW_PAGE_ATTRIBUTES).length).to eq(9)
  end
  ...
  it "#display_resource returns a string" do
    expect(subject.new.display_resource(status)).to match "^Status ##{status.to_param}$"
  end
end
