...
describe 'Device Types admin interface', type: :feature do
  let(:user) { build :user }
  let(:admin) { build :user, :admin }

  describe 'unauthenticated user attempts access' do
    before { visit admin_device_types_path }
  ...
  describe 'admin user attempts access' do
    ...
    it { expect(page).to have_current_path(admin_device_types_path) }
    it { expect(page).to have_content 'Device Types' }
  end
end
