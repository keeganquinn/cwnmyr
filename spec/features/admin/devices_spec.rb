...
describe 'Devices admin interface', type: :feature do
  let(:user) { build :user }
  let(:admin) { build :user, :admin }

  describe 'unauthenticated user attempts access' do
    before { visit admin_devices_path }
  ...
  describe 'admin user attempts access' do
    ...
    it { expect(page).to have_current_path(admin_devices_path) }
    it { expect(page).to have_content 'Devices' }
  end
end
