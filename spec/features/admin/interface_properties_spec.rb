...
feature 'Interface Properties admin interface', :devise do
  let (:user) { build :user }
  ...
  scenario "admin user attempts access" do
    login_as admin
    visit admin_interface_properties_path
    expect(current_path).to eq(admin_interface_properties_path)
    expect(page).to have_content 'Interface Properties'
  end
end
