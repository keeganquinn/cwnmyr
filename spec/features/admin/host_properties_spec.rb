...
describe 'Host Properties admin interface', type: :feature do
  let(:user) { build :user }
  let(:admin) { build :user, :admin }

  describe 'unauthenticated user attempts access' do
    before { visit admin_host_properties_path }
  ...
  describe 'admin user attempts access' do
    ...
    it { expect(page).to have_current_path(admin_host_properties_path) }
    it { expect(page).to have_content 'Host Properties' }
  end
end
