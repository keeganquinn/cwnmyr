...
Control.ColorPicker.prototype = {
  ...
  open : function(event) {
	...
	Event.observe(document, 'keypress', this.handleKeyPress);
	Event.observe(document, "mousedown", this.handleMouseDown);
  ...
  close : function(event) {
	...
	Event.stopObserving(document, 'keypress', this.handleKeyPress);
	Event.stopObserving(document, "mousedown", this.handleMouseDown);
  ...
  updateFromFieldValue : function(event) {
	...
	var rgb = YAHOO.util.Color.hex2rgb((field != null && field.value != null && field.value != "") ? field.value : "000000");
    if (!YAHOO.util.Color.isValidRGB(rgb)) return;
    var hsv = YAHOO.util.Color.rgb2hsv( rgb[0], rgb[1], rgb[2] );
	if (hsv[0] == undefined) hsv[0] = 0;
	if (hsv[1] == undefined) hsv[1] = 0;
	if (hsv[2] == undefined) hsv[2] = 0;
  ...
  updateSwatch : function() {
	var rgb = YAHOO.util.Color.hex2rgb((this.field != null && this.field.value != null && this.field.value != "") ? this.field.value : "000000");
    if (!YAHOO.util.Color.isValidRGB(rgb)) return;
  ...
  handleKeyPress : function(event) {
	if(event.keyCode != Event.KEY_ESC || Control.ColorPicker.activeColorPicker == null)
		return;

	var picker = Control.ColorPicker.activeColorPicker;
	if (picker != null) {
		picker.isCancel = true;
		picker.close();
	}
  },
  handleMouseDown : function(event) {
	var target = (event != null) ? Event.element(event) : null;
	if (target == null || ["colorpicker", "colorpicker-footer", "colorpicker-okbutton", "colorpicker-value-input"].include(target.id))
		return;
